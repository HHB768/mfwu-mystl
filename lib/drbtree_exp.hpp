#ifndef __DRBTREE_EXP_HPP__
#define __DRBTREE_EXP_HPP__

#ifdef __GLOBAL_DEBUG__
#   define __DRBTREE_EXP_DEBUG__
#else
#   ifdef __UNIT_TEST_RBTREE_BRIEF__
#       undef __DRBTREE_EXP_DEBUG__
...
template <typename Key, typename Value, 
          typename CmpFunctor=mfwu::less<Key>>
class drbtree_exp {
public:
    ...
    drbtree_exp() : root_(nullptr) {}
    drbtree_exp(const std::initializer_list<value_type>& vals) : root_(nullptr) {
        for (auto&& val : vals) { 
            insert(val); 
        }
    }
    template <typename InputIterator,
             ...
             >
    drbtree_exp(InputIterator first, InputIterator last) : root_(nullptr) {
        for ( ; first != last; ++first) {
            insert(*first);
        }
    }
    drbtree_exp(const drbtree_exp& rbt) {
        this->copy(rbt);
    }
    drbtree_exp(drbtree_exp&& rbt) : root_(rbt.root_) {
        rbt.root_ = nullptr;
    }
    ~drbtree_exp() {
    ...
    drbtree_exp& operator=(const drbtree_exp& rbt) {
        this->destroy_tree(root_);
        this->copy(rbt);
        return *this;
    }
    drbtree_exp& operator=(drbtree_exp&& rbt) {
        root_ = rbt.root_;
        rbt.root_ = nullptr;
        return *this;
    }
    ...
    CmpFunctor get_cmp() const { return cmp; }
    ...
    void erase(node* root) {
        ...
        if (root->color == red) {
#ifdef __DRBTREE_EXP_DEBUG__
            std::cout << "erase red: " << root->val << "\n";
#endif  // __DRBTREE_EXP_DEBUG__
...
private:
    void copy(const drbtree_exp& rbt) {
...
};  // endof class drbtree_exp

// template <typename T, typename CmpFunctor>
// CmpFunctor drbtree_exp<T, CmpFunctor>::cmp = {};
